...
void MAXreg_wr(BYTE reg, BYTE val) {
	//psuedocode:
	//select MAX3421E 
	XSpi_SetSlaveSelect(&SpiInstance, MAX_MASK);
	...
	int code = XSpi_Transfer(&SpiInstance, tx_buf, NULL, count);
	//if return code != 0 print an error
	if (code != 0) {
		xil_printf("MAXreg_wr code error. \n");
	}
	//deselect MAX3421E (may not be necessary if you are using SPI peripheral)
	XSpi_SetSlaveSelect(&SpiInstance, MAX_DESELECT_MASK);
...
BYTE* MAXbytes_wr(BYTE reg, BYTE nbytes, BYTE* data) {
    ...
    memcpy(&tx_buf[1], data, nbytes);
	//read return code from SPI peripheral 
	int code = XSpi_Transfer(&SpiInstance, tx_buf, NULL, (unsigned int)(nbytes + 1));
	//if return code != 0 print an error
	if (code != 0) {
		xil_printf("MAXbytes_wr code error. \n");
	}
	//deselect MAX3421E (may not be necessary if you are using SPI peripheral)
	XSpi_SetSlaveSelect(&SpiInstance, MAX_DESELECT_MASK);
	//return (data + nbytes);
...
BYTE MAXreg_rd(BYTE reg) {
	...
	u8 rx_buf[count];
	//read return code from SPI peripheral 
	int code = XSpi_Transfer(&SpiInstance, tx_buf, rx_buf, count);
	//if return code != 0 print an error
	if (code != 0) {
		xil_printf("MAXreg_rd code error. \n");
	}
	//deselect MAX3421E (may not be necessary if you are using SPI peripheral)
	XSpi_SetSlaveSelect(&SpiInstance, MAX_DESELECT_MASK);
	//return val
...
BYTE* MAXbytes_rd(BYTE reg, BYTE nbytes, BYTE* data) {
	...
	//write reg via SPI
	tx_buf[0] = reg;
	memset(&tx_buf[1], 0, nbytes);
	//read data[n] from SPI, where n goes from 0 to nbytes-1
	u8 rx_buf[nbytes + 1];
	//read return code from SPI peripheral 
	int code = XSpi_Transfer(&SpiInstance, tx_buf, rx_buf, nbytes + 1);
	//if return code != 0 print an error
	if (code != 0) {
		xil_printf("MAXbytes_rd code error. \n");
	}
	//deselect MAX3421E (may not be necessary if you are using SPI peripheral)
	XSpi_SetSlaveSelect(&SpiInstance, MAX_DESELECT_MASK);
	//return (data + nbytes);
    memcpy(data, &rx_buf[1], nbytes);
	return (data + nbytes);
}
