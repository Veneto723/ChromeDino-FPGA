...
void setColorPalette (uint8_t color, uint8_t red, uint8_t green, uint8_t blue) {
    ...
    // Pack RGB values into a 12-bit value
    uint32_t data = (red << 8) | (green << 4) | blue;
    uint8_t shift = (color % 2 == 0) ? 1 : 13;

    hdmi_ctrl->palette[color / 2] = (hdmi_ctrl->palette[color / 2] & ~(0xFFF << shift)) | (data << shift);
}
